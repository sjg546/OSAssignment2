...
void LRU(std::vector<std::string> traces, int pagecount, int pageSize)
{
    ...
    while (i < traces.size())
    {
        ...
        if (traces[i][0] != '#')
        {
            ...
            if (memory.size() < pagecount)
            {
                ...
                {
                    int inMemorytemp = Utilities::checkIsInMemory(memory, pagecount, traces[i]);
                    memory[inMemorytemp]->worked = 0;
                }
            }
            else
            {
                ...
                {
                    int oldestItem = Utilities::getOldestWorked(memory);
            ...
            Utilities::incrementWorked(memory);
